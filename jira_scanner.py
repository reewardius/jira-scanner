...
# ─────────────────────────────────────────────
# Personal notifications per author
# ─────────────────────────────────────────────
def send_personal_notifications(all_findings: List[Dict], email_config: Dict,
                                 allowed_domains: List[str]) -> Dict:
    """
    Group findings by author email and send each author a personal XLSX report
    containing only their own leaks.

    Args:
        all_findings:    Full list of findings from the scan.
        email_config:    Dict with sender, aws_region keys.
        allowed_domains: Only send to addresses whose domain is in this list
                         (e.g. ['fozzy.ua', 'temabit.com']).
                         Empty list = allow all domains.

    Returns:
        Dict with keys: sent (int), skipped_domain (int), failed (int),
                        skipped_no_email (int)
    """
    if not AWS_SES_AVAILABLE:
        print("❌ AWS SES not available — personal notifications skipped.")
        return {}

    sender = email_config.get('sender')
    aws_region = email_config.get('aws_region', 'eu-central-1')

    # Normalize domains to lowercase, strip leading @
    allowed = [d.lstrip('@').lower() for d in allowed_domains] if allowed_domains else []

    # Group findings by author_email
    by_author: Dict[str, List[Dict]] = {}
    for f in all_findings:
        email_addr = f.get('author_email', 'N/A')
        if not email_addr or email_addr == 'N/A':
            continue
        by_author.setdefault(email_addr, []).append(f)

    stats = {'sent': 0, 'skipped_domain': 0, 'failed': 0, 'skipped_no_email': 0}
    stats['skipped_no_email'] = len(set(f.get('author', '') for f in all_findings
                                         if not f.get('author_email') or f.get('author_email') == 'N/A'))

    print(f"\n📨 Sending personal notifications to {len(by_author)} author(s)…")

    ses_client = boto3.client("ses", region_name=aws_region)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    for author_email, findings in by_author.items():
        # Domain check
        domain = author_email.split('@')[-1].lower() if '@' in author_email else ''
        if allowed and domain not in allowed:
            print(f"   ⏭️  Skipped (domain not allowed): {author_email}")
            stats['skipped_domain'] += 1
            continue

        author_name = findings[0].get('author', author_email)
        n = len(findings)
        issues = sorted(set(f['issue_key'] for f in findings))

        # Generate personal XLSX in memory via temp file
        tmp_xlsx = f"/tmp/personal_{author_email.replace('@','_').replace('.','_')}_{timestamp}.xlsx"
        try:
            create_secrets_report(findings, tmp_xlsx)

            # Build email
            subject = (
                f"⚠️ Security Alert: {n} exposed secret{'s' if n != 1 else ''} "
                f"found in your Jira issue{'s' if len(issues) != 1 else ''}"
            )

            issues_str = ', '.join(issues[:10])
            if len(issues) > 10:
                issues_str += f' and {len(issues) - 10} more'

            greeting = f"Hi {author_name}," if author_name and author_name.lower() != "unknown" else "Hi,"
            body = f"""{greeting}

Our automated security scanner has detected {n} exposed secret{'s' if n != 1 else ''} in Jira issue{'s' if len(issues) != 1 else ''} you created or commented on:
...
This is an automated message from the Jira Secrets Scanner."""
            ...
            encoders.encode_base64(part)
            part.add_header('Content-Disposition', 'attachment; filename=your_jira_secrets.xlsx')
            msg.attach(part)
            ...
            print(f"   ✅ Sent to {author_email} ({n} finding{'s' if n != 1 else ''}, issues: {issues_str})")
            stats['sent'] += 1

        except Exception as e:
            print(f"   ❌ Failed to send to {author_email}: {e}")
            stats['failed'] += 1
        finally:
    ...
    if stats['skipped_domain']:
        print(f"   ⏭️  Skipped (domain) : {stats['skipped_domain']}")
    if stats['skipped_no_email']:
        print(f"   ⚠️  No email in Jira : {stats['skipped_no_email']}")
    if stats['failed']:
...
Examples:
  ...
  python jira_scanner.py --env --scan-secrets --workers 20
    ...
    parser.add_argument('--notify-authors', action='store_true',
                        help='Send a personal email to each Jira issue author with only their own findings')
    parser.add_argument('--notify-domain', type=str, default=None,
                        help='Only notify authors whose email matches one of these domains '
                             '(comma-separated, e.g. fozzy.ua,temabit.com). '
                             'Authors with other domains (e.g. gmail.com) are silently skipped.')
...
def main():
    ...
    # ── Personal notifications per author ──
    if args.notify_authors and all_findings:
        if not args.email_sender:
            print("❌ --notify-authors requires --email-sender")
        else:
            allowed_domains = (
                [d.strip() for d in args.notify_domain.split(',') if d.strip()]
                if args.notify_domain else []
            )
            if allowed_domains:
                print(f"\n🌐 Allowed domains for notifications: {', '.join(allowed_domains)}")
            else:
                print("\n⚠️  No domain filter set — notifications will be sent to all author emails")
            email_config = {
                'sender': args.email_sender,
                'aws_region': args.aws_region,
            }
            send_personal_notifications(all_findings, email_config, allowed_domains)
    elif args.notify_authors and not all_findings:
        print("\nℹ️  --notify-authors: no findings to notify about")
